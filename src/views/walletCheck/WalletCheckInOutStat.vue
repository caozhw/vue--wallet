  ...
  <section>
    ...
    <el-col :span="24" class="" style="padding-bottom: 0px;margin-top: 20px;">
      <template >
        <el-tabs v-model="biz_type" type="card" @tab-click="handleTabClick">
          <el-tab-pane label="充币对账"  name="0"></el-tab-pane>
          <el-tab-pane label="提币对账"  name="1"></el-tab-pane>
        </el-tabs>
      </template>
    </el-col>
    <!--列表 fromBookAddress-->
    <el-col :span="24" class="" style="padding-bottom: 0px;margin-top: 0;">
      ...
      :summary-method="getSummaries"  show-summary  :fit="tableFit" highlight-current-row border   style="width: 100%;" :row-style="rowColor"  :cell-style="cellStyle" @cell-click="sumTransMoneyClick">
        ...
        <el-table-column prop="sumTransMoney" label="交易平台充币总量" width="80" align="right" :formatter="namberFormatter">
        </el-table-column>
        <el-table-column prop="sumWalletMoney" label="钱包系统充币总量" width="80" align="right" :formatter="namberFormatter">
        </el-table-column>
      ...
      <el-table :data="listData"   v-if="biz_type == 1"
       :summary-method="getSummaries"  show-summary  :fit="tableFit" highlight-current-row border   style="width: 100%;" :row-style="rowColor"  :cell-style="cellStyle" @cell-click="sumTransMoneyClick">
        <el-table-column prop="check_day" label="日期" width="120" align="center" >
        </el-table-column>
        <el-table-column prop="wallet_type" label="币种" width="80"  align="center" :formatter="walletTypeFormatter">
        ...
        <el-table-column prop="sumTransMoney" label="交易平台提币总量" width="80" align="right" :formatter="namberFormatter">
        </el-table-column>
        <el-table-column prop="sumWalletMoney" label="钱包系统提币总量" width="80" align="right" :formatter="namberFormatter">
        </el-table-column>
  ...
  export default{
    data:function(){
      return{
        ...
        //查询集合
        form:{
          api_method:'WalletCheckInStat',//充币对账   WalletCheckOutStat提币对账
    ...
    methods: {
      ...
      if(this.biz_type == 0){
        this.form.api_method = 'WalletCheckInStat';
      }else if(this.biz_type == 1){
        this.form.api_method = 'WalletCheckOutStat';
      }
      ...
      cellStyle({row, column, rowIndex, columnIndex}){ //单元格样式
        if(column.label == '交易平台充币总量' || column.label == '交易平台提币总量'){ 
      ...
      sumTransMoneyClick(row, column, cell, event){ //单元格点击事件
        if(column.label == '交易平台充币总量' || column.label == '交易平台提币总量'){
          console.log(row);
          if(this.biz_type == 0){
            this.$router.push({ path: '/WalletCheckInList', query: { wallet_type: row.wallet_type,check_day:row.check_day } }); 
          }else{
            this.$router.push({ path: '/WalletCheckOutList', query: { wallet_type: row.wallet_type,check_day:row.check_day } }); 
          }
